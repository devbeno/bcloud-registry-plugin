...
describe BcloudPlugins::Registry::Command::Push do
  include_context "vagrant-unit"

  let(:env) { isolated_environment.create_bcloud_env }
  let(:server_url) { "http://foo.com" }

  subject { described_class.new(argv, env) }

  let(:uploader) { double("uploader") }
  let(:client) { BcloudPlugins::Registry::Client.new(env, server_url) }
  ...
  describe "#execute" do
    ...
    context "with invalid URL" do
      let(:argv) { ["test.box", "http@//bclop.com", "0.1.0", "virtualbox"] }

      it "shows error message" do
        expect { subject.execute }
            .to raise_error(BcloudPlugins::Registry::Errors::InvalidURL)
      end
    end
    ...
    context "with box name instead of URL" do
      ...
      context "when environment variable is set" do
        ...
        it "constructs valid URL" do
          expect(BcloudPlugins::Registry::Uploader).to receive(:new).with(
              env,
              box_file.to_s,
              "#{server_url}/user/box",
              "0.1.0",
    ...
    context "when user not logged in" do
      ...
      before do
        client = double(BcloudPlugins::Registry::Client)
        allow(BcloudPlugins::Registry::Client)
      ...
      it "shows error message" do
        expect { subject.execute }
            .to raise_error(BcloudPlugins::Registry::Errors::NotLoggedIn)
      end
    end
